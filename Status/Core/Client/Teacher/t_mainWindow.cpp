...
TMainWindow::TMainWindow(QWidget *parent): QMainWindow(parent) {
	...
	// 信号绑定
	this->connect(this->m_ui.full_or_normal_btn, &QPushButton::clicked, this, &TMainWindow::setWindowFull);
}

void TMainWindow::setWindowFull() {
	QRect window = QApplication::desktop()->availableGeometry();
	this->setGeometry(-5, -5, window.width() + 10, window.height() + 10);

	// 更换槽函数
	this->disconnect(this->m_ui.full_or_normal_btn, &QPushButton::clicked, this, &TMainWindow::setWindowFull);
	this->connect(this->m_ui.full_or_normal_btn, &QPushButton::clicked, this, &TMainWindow::setWindowNormal);
...
void TMainWindow::setWindowNormal() {
	QRect window = QApplication::desktop()->availableGeometry();
	...
	// 更换槽函数
	this->disconnect(this->m_ui.full_or_normal_btn, &QPushButton::clicked, this, &TMainWindow::setWindowNormal);
	this->connect(this->m_ui.full_or_normal_btn, &QPushButton::clicked, this, &TMainWindow::setWindowFull);
...
void TMainWindow::mouseMoveEvent(QMouseEvent *event) {
	// 计算鼠标当前位置
...
void TMainWindow::mousePressEvent(QMouseEvent *event) {
...
void TMainWindow::mouseReleaseEvent(QMouseEvent *event) {
	this->is_pressed = false;
	this->is_moving = false;
	this->is_resizing = false;
...
void TMainWindow::mouseDoubleClickEvent(QMouseEvent *event) {
	if (this->start_pos.x() < this->width() && this->start_pos.y() < 40) {
		// 窗口已经最大化
