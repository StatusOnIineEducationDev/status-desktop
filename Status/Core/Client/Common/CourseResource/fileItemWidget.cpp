...
FileItemWidget::FileItemWidget(const QString &course_id, const QString &file_id,
	const QString &resource_title, const QString &filename, const int &upload_timestamp, 
	const QString &uploader, const int &file_size, QWidget *parent)
	: m_course_id(course_id), m_file_id(file_id), m_resource_title(resource_title), 
	m_filename(filename), m_upload_timestamp(upload_timestamp), m_uploader(uploader), 
	m_file_size(file_size), QWidget(parent) {
	m_ui.setupUi(this);
	...
	this->m_ui.title_text->setText(resource_title);
